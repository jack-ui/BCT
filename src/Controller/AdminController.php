...
//------------------------------------GESTION DES USERS -----------------------------------------
    ...
    public function deleteUser($id, ObjectManager $manager)
    {
        ...
        $user = $manager -> find(User::class, $id);
            ...
            if($user -> getBoutiqueId() != NULL){

              $this -> addFlash('danger',  'Le profil ' .  $user->getUsername() . ' ne peut pas être supprimé, car il est propriétaire d\'une boutique active. Veuillez d\'abord supprimer la boutique n°' . $user -> getBoutiqueId() -> getId() . ' afin de pouvoir supprimer le profil');
            }
            else{
			          $manager -> remove($user);
		            $manager -> flush();
    ...
    public function deleteShop(ObjectManager $manager, $id)
    {
        ...
        if($boutique)
        {
            foreach($boutique -> getProduits() as $produit){
              $pcs = $manager -> getRepository(ProduitCommande::class) -> findAll();
              foreach($pcs as $pc){
                $manager -> remove($pc);
                $manager -> remove($pc -> getCommande());
              }
              $manager -> remove($produit);
            }
            $boutique -> getUserId() -> setBoutiqueId(NULL);
